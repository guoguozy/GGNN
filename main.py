...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        # train_orig = pd.read_csv(f'./data/{dataset}/wadi_train_demo.csv', sep=',', index_col=0)
        # test_orig = pd.read_csv(f'./data/{dataset}/wadi_test_demo.csv', sep=',', index_col=0)
        ...
        # fc_struc = get_prior_graph_struc(dataset)

        set_device(env_config['device'])
        self.device = get_device()

        fc_edge_index = build_loc_net(fc_struc, list(train.columns), feature_map=feature_map)
        fc_edge_index = torch.tensor(fc_edge_index, dtype = torch.long) # 边序号对应 2 * edges_num
        ...
        self.model = Model(edge_index_sets, len(feature_map), 
                batch_num=train_config['batch'],
    ...
    def get_save_path(self, feature_name=''):
        ...
        datestr = self.datestr.replace('|', '-').replace(':', '-')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-topk', help='topk num', type = int, default=25)
